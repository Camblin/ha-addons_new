...
class BestinRS485 {
    constructor() {
        ...
        if (options.server_enable) {
            ...
            this.shouldRegisterSrvEV = options.server_type === "v2" && options.server.ev_registration;
            this.hasSmartLighting = options.server_type === "v2" && options.smart_lighting;
            this.lightStatusInterval = null;
        } else {
    ...
    createMqttClient() {
        const client = mqtt.connect({
            ...
            clientId: 'bestin_wallpad-' + Date.now(),
        ...
        client.on('connect', () => {
            ...
            if (this.shouldRegisterSrvEV && fs.existsSync('./session.json')) {
                ...
                logger.debug(`Setting up elevator device: ${JSON.stringify(fs.readFileSync('./session.json'))}`);
    ...
    mqttCommand(topic, message) {
        ...
        logger.debug(`Checking Session - Is Server Enabled: ${this.isServerEnabled}, Session File Data: ${JSON.stringify(this.checkSessionFile())}`);
    ...
    findCommandIndex(packet) {
        ...
        for (let i = 0; i < serialCommandQueue.length; i++) {
            ...
            logger.debug(`Serial command - 2BYTE-P1: [${secondByte}|${packet[1]}] ACKHEX-PI: [${ackHex}|${packetI}]`);
    ...
    addCommandToQueue(cmdHex, device, room, name, value, callback) {
        ...
        logger.debug(`Serial Queue: ${serialCmd}`);
    ...
    processCommand(serialCmd) {
        ...
        logger.debug(`Set write device: ${writeHandle}`);
        try {
    ...
    performDiscovery(device, room, name, discoveryTimeout = 20000) {
        const formatAndExecuteDiscovery = () => {
            if (!this.discoveryScheduled && options.mqtt.discovery) {
                ...
                if (device === "elevator") {
                    setTimeout(() => {
                        options.server.ev_registration = false;
                    }, 3000);
                }
            }
        }
    ...
    loginManagement(res, type, session) {
        ...
        const cookie = () => {
            ...
            logger.debug(`Server-login cookie: ${JSON.stringify(cookieMap)}`);
    ...
    async getServerLightStatus(url, type, session) {
        try {
            ...
            if (error.message.includes("(reading 'imap')")) {
                logger.debug(`Session expired.`);

                this.loginFunc("refresh", this.loginUrl);
                return;
            }
    ...
    parseJsonLightStatus(json) {
        ...
        if (Array.isArray(units)) {
            for (const unit of units) {
                logger.debug(`Smart-light status - Count: several, Units: ${units}`);
                processUnit(unit);
            }
        } else {
            logger.debug(`Smart-light status - Count: single, Units: ${units}`);
            processUnit(units);
        }
    }
    ...
    serverCommand(topic, value, json) {
        if (topic[1] === "light" || (topic[1] === "slight" && this.smartLightDataArray?.length > 0)) {
            ...
            const evaddress = recursiveFormatWithArgs(V2ELEVATORCMD, json.url, options.server.address, options.server.uuid);
            const defaultMsg = "Registration failed! Default address. Please enter generation wallpad IP";
            options.server.address === "10.1.10.1" ? logger.error(defaultMsg) : this.serverEvCommand(evaddress, json);
        }
    }
